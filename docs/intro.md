...
It offers best-in-class DX through strong typing and easy separation of concerns, and is
...
Alternatively, you can opt to register custom stage functions on the factory.
...
:::tip
When calling `stage('render')`, `stage` will return a function component instead of a hook.
In fact, calling `stage(field)` on any field returning a React node will create a function component
instead of a hook.
:::
